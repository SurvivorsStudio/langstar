    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {providers.map((provider, index) => (
          <div
            key={provider.id}
            className={`${provider.color} border-2 rounded-xl p-6 cursor-pointer transform transition-all duration-300 hover:scale-105 hover:shadow-lg group animate-scaleIn`}
            style={{
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <h3 className={`text-xl font-semibold ${provider.textColor} mb-1 transition-colors duration-300`}>
                      {provider.name}
                    </h3>
                    <div className="flex items-center text-sm text-gray-600">
                      <span className="bg-white px-2 py-1 rounded-full text-xs font-medium transition-all duration-300 group-hover:bg-opacity-90">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <ArrowRight 
                    className={`w-5 h-5 ${provider.textColor} opacity-0 group-hover:opacity-100 transform group-hover:translate-x-1 transition-all duration-300`} 
                  />
                </div>
              </div>
            </div>
