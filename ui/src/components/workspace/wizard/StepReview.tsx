    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white rounded-xl shadow-lg p-8 animate-scaleIn">
        <div className="flex items-center mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Review & Save Connection</h2>
            <p className="text-gray-600">Review your configuration and save the connection</p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <span className="ml-2 text-gray-900">{provider?.name}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="ml-2 text-gray-900">{provider?.category}</span>
            </div>
            <div>
              <span className="font-medium text-gray-600">Max Tokens:</span>
              <span className="ml-2 text-gray-900">{model?.maxTokens?.toLocaleString() || 'N/A'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">Authentication Details</h3>
            <button
          ...
          <div className="space-y-3 text-sm">
            {provider?.id === 'aws' ? (
              <>
                <div>
                  <span className="font-medium text-gray-600">Access Key ID:</span>
                  <span className="ml-2 text-gray-900">
                    {showSensitiveData ? formData.accessKeyId : maskSensitiveData(formData.accessKeyId ?? '')}
                  </span>
                </div>
                <div>
                  <span className="font-medium text-gray-600">Secret Access Key:</span>
                  <span className="ml-2 text-gray-900">
        ...
        <div className="flex justify-between">
          ...
          <button
            ...
            className="flex items-center px-6 py-3 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-200"
          >
