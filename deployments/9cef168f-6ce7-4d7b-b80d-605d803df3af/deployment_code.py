...
# Deployment ID: 9cef168f-6ce7-4d7b-b80d-605d803df3af
# Generated at: 2025-08-18T00:56:46.349196
...
class MyState(BaseModel):
    ...
    End :dict = {}
    Prompt_Config :dict = {'config': {'template': '작성된 프롬포트는 다음과 같습니다\n\nUser: {user_input}\nAssistant:'}, 'outputVariable': 'user_input', 'node_type': 'promptNode', 'next_node': [{'node_name': 'Agent', 'node_type': 'agentNode'}, {'node_name': 'my_function', 'node_type': 'userNode'}], 'node_name': 'Prompt'}
    Prompt :dict = {}
    Agent_Config :dict = {'config': {'model': {'connName': 'nova-lite', 'providerName': 'aws', 'modelName': 'amazon.nova-lite-v1:0', 'accessKeyId': 'adfadf', 'secretAccessKey': 'asfasdf', 'region': 'afdadf'}, 'tools': [], 'userPromptInputKey': 'user_input', 'systemPromptInputKey': 'system_prompt', 'agentOutputVariable': 'agent_response', 'topK': 40, 'topP': 1, 'temperature': 0.6, 'maxTokens': 1000, 'node_type': 'agentNode', 'next_node': [{'node_name': 'End', 'node_type': 'endNode'}], 'node_name': 'Agent', 'chat_history': []}}
    Agent :dict = {}
...
graph.add_edge("_Prompt", "_my_function")
graph.add_edge("_my_function", "_Agent")
...
# 배포 실행 함수 (로컬 실행)
def run_deployment_9cef168f_6ce7_4d7b_b80d_605d803df3af(input_data):
    try:
        result = app.invoke(input_data, {"configurable": {"thread_id": 1}})
        return {
            "success": True,
            "deployment_id": "9cef168f-6ce7-4d7b-b80d-605d803df3af",
            "result": result
        }
    except Exception as e:
        return {
